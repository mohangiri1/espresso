...
cd /data/girim/Research/GaBi/calculation/BC/
...
#summing for the Bi s orbital:
sumpdos.x *\(Bi\)*\(s_j*\) > atom_Bi_s.dat
cp atom_Bi_s.dat ./figure/atom_Bi_s.dat

#summing for the Bi p orbital:
sumpdos.x *\(Bi\)*\(p_j*\) > atom_Bi_p.dat
cp atom_Bi_p.dat ./figure/atom_Bi_p.dat
...
def Bi_s_p_pdos(Bi_s, Bi_p, fermi_energy):
    """
    Plot the Projected Density of States (PDOS) for As atoms, considering s and p orbitals.

    Args:
    Bi_s (str): Path to the file containing Bi s PDOS data.
    Bi_p (str): Path to the file containing Bi p PDOS data.
    fermi_energy (float): Fermi energy.
    ...
    # Plot Bi s PDOS
    energy, pdos = read_data(Bi_s, fermi_energy)
    plt.plot(energy, pdos, linewidth=0.75, color='red', label='Bi s')

    # Plot Bi p PDOS
    energy, pdos = read_data(Bi_p, fermi_energy)
    plt.plot(energy, pdos, linewidth=0.75, color='black', label='Bi p')
    ...
    plt.ylim(0, )
    plt.legend()
    plt.savefig('./figure/Bi_s_p_pdos.pdf')
    plt.show()
    
def GaAs_s_p_pdos(Ga_s, Ga_p, Bi_s, Bi_p, fermi_energy):
    """
    Plot the Projected Density of States (PDOS) for Ga and Bi atoms, considering s and p orbitals.
    ...
    # Plot Bi s PDOS
    energy, pdos = read_data(Bi_s, fermi_energy)
    plt.plot(energy, pdos, linewidth=0.75, color='green', label='Bi s')

    # Plot As p PDOS
    energy, pdos = read_data(Bi_p, fermi_energy)
    plt.plot(energy, pdos, linewidth=0.75, color='blue', label='Bi p')
    ...
    plt.ylim(0, )
    plt.legend()
    plt.savefig('./figure/GaBi_s_p_pdos.pdf')
    plt.show()
...
Bi_p = 'atom_Bi_p.dat'
Bi_s = 'atom_Bi_s.dat'
Bi_s_p_pdos(Bi_s, Bi_p, fermi_energy)

#Pdos of Ga and As in GaBi
GaBi_s_p_pdos(Ga_s, Ga_p, Bi_s, Bi_p, fermi_energy)

#Total Pdos and Total Dos in GaBi
pdos_tot = 'GaBi.pdos_tot'
...
output_figure_name = 'bandstructure'
fermi_energy = 0.1
