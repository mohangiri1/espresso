...
#PBS -o full_calculation.out
#PBS -e full_calculation.err
#PBS -N fcalc
...
mkdir full_calculation; cd full_calculation
...
# Run Quantum ESPRESSO (pw.x) with mpiexec using -bootstrap ssh
mpiexec -bootstrap ssh -np $NUM_PROCESSES -hostfile $HOSTFILE pw.x < $calculation.in > $calculation.out
#Copy the pure nscf calculation output in a directory called 'output_nscf'
cp -r output output_$calculation
...
mpiexec -bootstrap ssh -np $NUM_PROCESSES -hostfile $HOSTFILE pw.x < $calculation.in > $calculation.out
cp $calculation.out ./figure/$calculation.out
#Copy the pure bands calculation output in a directory called 'output_bands'
cp -r output output_$calculation
...
# Run Quantum ESPRESSO (pw.x) with mpiexec using -bootstrap ssh
mpiexec -bootstrap ssh -np $NUM_PROCESSES -hostfile $HOSTFILE bands.x < bands.kp.in > bands.kp.out
